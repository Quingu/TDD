...
public class Placar {
	...
	public Set<Ponto> pontosPorUsuario(String nomeUsuario) {
		return armazenamento.pontosPorUsuario(nomeUsuario).stream().filter(ponto -> ponto.getQuantidade() > 0)
	...
	private Comparator<Usuario> comparadorPorMaiorPontuacao(String tipoPonto) {
		return (usuario1, usuario2) -> Integer.compare(usuario2.quantidadePontosPorTipo(tipoPonto),
				usuario1.quantidadePontosPorTipo(tipoPonto));
	}
